          ...
          for (var menu in menu_list) {
            ...
            if (menu_list[menu].children && menu_list[menu].children.length > 0) {
              ...
              if (menu_list[menu].selected == 1) {
                ...
                jq(li).append('<input class="menuIds" disabled="false" type="checkbox"  checked="checked"  name="" id="' + menu_list[menu].id + '"/>' + '<span>' + menu_list[menu].name +
                  '</span>').append(
                ...
                var li = jq('<li></li>')
                //将li的文本设置好，并马上添加一个空白的ul子节点，并且将这个li添加到父亲节点中
                jq(li).append('<input class="menuIds" disabled="false" type="checkbox" name="" id="' + menu_list[menu].id + '"/>' + '<span>' + menu_list[menu].name +
                  '</span>').append(
                  '<ul></ul>').appendTo(parent)
                //将空白的ul作为下一个递归遍历的父亲节点传入
                showall(menu_list[menu].children, jq(li).children().eq(2))
              ...
              //遍历按钮权限
              if (menu_list[menu].selected == 1) {
                jq('<li><input type="checkbox" disabled="false"  checked="checked"  class="menuIds" name="" id="' + menu_list[menu].id + '"/></li>').append('<span>' + menu_list[menu].name +
                  '</span>').appendTo(
                  parent)
              } else {
                jq('<li><input type="checkbox" disabled="false"  class="menuIds"  name="" id="' + menu_list[menu].id + '"/></li>').append('<span>' + menu_list[menu].name +
                  '</span>').appendTo(
                  parent)
              }
            }
            ...
            if (liDom.eq(i).children('input:checkbox').attr('id') == _this.privilegesID[k]) {
              ...
              for (let j = 0; j < _this.privilegesArr[k].length; j++) {
                ...
                if (_this.privilegesArr[k][j].selected == 1) {
                  liDom.eq(i).append('<ul><li><input type="checkbox"  disabled="false" checked="checked" class="privilegeIds" name="" id="' + _this.privilegesArr[k][j].id + '"/>' + '<span>' + _this.privilegesArr[k][j].name + '</span></li></ul>')
                } else {
                  liDom.eq(i).append('<ul><li><input type="checkbox"  disabled="false" class="privilegeIds" name="" id="' + _this.privilegesArr[k][j].id + '"/>' + '<span>' + _this.privilegesArr[k][j].name + '</span></li></ul>')
                }
